...
const ProjectCard: React.FC<{ project: Project; onSelect: () => void; onDelete: () => void; }> = ({ project, onSelect, onDelete }) => {
    return (
        <div className="bg-brand-surface p-6 rounded-lg shadow-lg flex flex-col justify-between">
            <div>
                <h3 className="text-xl font-medium text-brand-on-surface mb-2">{project.name}</h3>
                <p className="text-brand-on-surface/70 italic mb-4">"{project.query}"</p>
                <div className="flex space-x-4 text-brand-on-surface/80 mb-4">
                    <span>{project.videos.length} Videos</span>
                    <span>{project.shorts.length} Shorts</span>
                </div>
                <p className="text-sm text-brand-on-surface/50">
            ...
            <div className="mt-6 flex space-x-2">
                <button
                    onClick={onSelect}
                    className="flex-1 bg-brand-accent text-white font-medium uppercase text-sm tracking-wider py-2 px-4 rounded-lg shadow-md hover:bg-brand-accent-dark transition-colors"
                >
                    Load Project
                </button>
                <button
                    onClick={(e) => { e.stopPropagation(); onDelete(); }}
                    className="bg-red-600 text-white font-medium uppercase text-sm tracking-wider py-2 px-4 rounded-lg shadow-md hover:bg-red-700 transition-colors"
                >
                    Delete
                </button>
            </div>
